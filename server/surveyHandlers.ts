...
async function upstashFetch(path: string, init?: RequestInit) {
  ...
  if (!response.ok || payload?.error) {
    throw new Error(
      `Upstash request failed (${response.status} ${response.statusText}): ${payload?.error ?? "Unknown error"}`
    );
  }
...
async function saveSurveyResponse(response: StoredSurveyResponse) {
  const key = `survey:response:${response.id}`;
  ...
  await upstashFetch("/pipeline", {
    ...
    body: JSON.stringify([
      ["SET", key, payload],
      ["LPUSH", "survey:responses", response.id],
    ]),
  });
}

async function listSurveyResponses(): Promise<StoredSurveyResponse[]> {
  const idsResult = await upstashFetch("/lrange/survey:responses/0/-1", {
    method: "POST",
  });
  const ids = Array.isArray(idsResult?.result) ? idsResult.result.filter(Boolean) : [];

  if (ids.length === 0) {
    return [];
  }

  const commands = ids.map((id: string) => ["GET", `survey:response:${id}`]);
  const rows = await upstashFetch("/pipeline", {
    method: "POST",
    body: JSON.stringify(commands),
  });

  return (Array.isArray(rows) ? rows : [])
    .map((row: { result?: string | null }) => row?.result)
    .filter((value): value is string => typeof value === "string" && value.length > 0)
    .map(value => JSON.parse(value) as StoredSurveyResponse);
}
...
function buildSurveyCsv(rows: StoredSurveyResponse[]): string {
  const headers = [
    "id",
    "submittedAt",
    "gender",
